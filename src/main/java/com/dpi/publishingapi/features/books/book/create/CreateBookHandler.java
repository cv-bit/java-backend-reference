...
public class CreateBookHandler implements Command.Handler<CreateBookRequest, CreateBookResponse> {
    ...
    public CreateBookResponse handle(CreateBookRequest createBookRequest) {
        ...
        book.setLanguages(new HashSet<>(createBookRequest.languages().stream().map(language -> new Language(ELanguage.valueOf(language.toUpperCase().strip()))).collect(Collectors.toList())));
        ...
        if (createBookRequest.difficulty().replaceAll("\\s", "").equalsIgnoreCase("INTERMEDIATE-ADVANCED")) {
            book.setDifficulty(Difficulty.INTERMEDIATEADVANCED);
        } else {
